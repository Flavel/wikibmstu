<?php
	...
	$str = htmlentities(file_get_contents("changepassword.html"));
	$str = str_replace ( "&lt;" , "<" , $str );
    $str = str_replace ( "&gt;" , ">" , $str );
    $str = str_replace ( "&quot;" , '"' , $str );
            ...
            if($result1['password'] == $_POST['password']) {
                if($_POST['newpassword1'] != ""){
                    if($_POST['newpassword1'] == $_POST['newpassword2']){
                        mysqli_query($linkUsr, "UPDATE `users` SET `password` = '" . $_POST['newpassword1'] . "' WHERE `id` = " . $_SESSION['id']);
                        mysqli_query($linkUsr, "INSERT INTO `notifications` (`userid`, `text`) VALUES (". $_SESSION['id'] .", 'Пароль успешно изменен')");
                    ...
                    $warning = 'Пароль не может быть пустым';
                }
            } else {
